...
if (isset($_POST['run_audit'])) {
    $mgr->runNightAudit($_SESSION['userId']);
}
$audits = $mgr->listNightAudits();

if (isset($_POST['generate_pdf'])) {
    $from = $_POST['pdf_from'] ?? date('Y-m-01');
    $to   = $_POST['pdf_to'] ?? date('Y-m-d');
    $mgr->generateFullReportPDF($from, $to);
    exit;
}

$page_title = 'Reports & Night Audit';
$active = 'reports';
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="row g-0">
        <div class="col-md-2 sidebar p-3">
            ...
            <nav class="nav flex-column">
                ...
                <a href="reports.php" class="nav-link active"><i class="bi bi-bar-chart me-2"></i>Reports</a>
        ...
        <div class="col-md-10">
            ...
            <div class="p-4">
                <form method="post" class="mb-4">
                    <button type="submit" name="run_audit" class="btn btn-warning"><i class="bi bi-moon"></i> Run Night Audit</button>
                </form>
                <div class="card shadow-sm mb-4">
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            <thead class="table-dark">
                                <tr><th>Date</th><th>Revenue</th><th>Payments</th><th>Occupied</th><th>Available</th><th>No-Shows</th></tr>
                            </thead>
                            <tbody>
                            <?php foreach ($audits as $a): ?>
                                <tr>
                                    <td><?= htmlspecialchars($a['audit_date']) ?></td>
                                    <td><?= number_format($a['total_revenue'] ?? 0, 2) ?></td>
                                    <td><?= number_format($a['total_payments'] ?? 0, 2) ?></td>
                                    <td><?= (int)($a['rooms_occupied'] ?? 0) ?></td>
                                    <td><?= (int)($a['rooms_available'] ?? 0) ?></td>
                                    <td><?= (int)($a['no_shows'] ?? 0) ?></td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
                        </table>
                ...
                <h5>Generate Full Report (PDF)</h5>
                <form method="post" action="reports.php" class="row g-2 mb-4">
                    <div class="col-md-3">
                        <input type="date" name="pdf_from" class="form-control" placeholder="From" required>
                    </div>
                    <div class="col-md-3">
                        <input type="date" name="pdf_to" class="form-control" placeholder="To" required>
                    </div>
                    <div class="col-md-3">
