...
if (isset($_POST['mark_read'])) {
    ...
    $db->update("UPDATE contact_messages SET is_read = 1 WHERE id = $msgId");
}

$messages = $db->select("SELECT * FROM contact_messages ORDER BY created_at DESC LIMIT 200");
    ...
    <div class="row g-0">
        <div class="col-md-2 sidebar p-3">
            ...
            <nav class="nav flex-column">
                ...
                <a href="messages.php" class="nav-link active"><i class="bi bi-envelope me-2"></i>Messages</a>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body p-0">
                        <?php if (empty($messages)): ?>
                            <p class="text-center py-4">No messages received.</p>
                        <?php else: ?>
                            <div class="list-group list-group-flush">
                            <?php foreach ($messages as $msg): ?>
                                <div class="list-group-item <?= empty($msg['is_read']) ? 'list-group-item-warning' : '' ?>">
                                    <div class="d-flex justify-content-between">
                                        <h5><?= htmlspecialchars($msg['name']) ?> <small class="text-muted">(<?= htmlspecialchars($msg['email']) ?>)</small></h5>
                                        <small><?= $msg['created_at'] ?></small>
                                    </div>
                                    <p class="mb-1"><strong><?= htmlspecialchars($msg['subject'] ?? 'No subject') ?></strong></p>
